...
|uuid |所属用户/组织id |bigint |null |不为空 |
|order_id |订单id |varchar |null |是 |
...
|status |0 初始化  1 支付成功   2.待转移(临时钱包) 3.已转移(临时钱包) |int |0 |不为空 |
|refund_status |退款状态  0 - 初始化;1 - 申请退款; 2 - 退款成功 ; 3 - 拒绝退款; |int |0 |不为空 |
|check_comment |审核评语 |varchar |null |是 |
|rmb_rate |当前 钱包费用比例 最大1 |decimal |0 |不为空 |
|tem_rmb_rate |当前 临时费用比例 最大1 |decimal |0 |不为空 |
|service_rate |当前 服务费用比例 最大1 |decimal |0 |不为空 |
|rmb_amount |钱包收取金额(分) |bigint |0 |不为空 |
|tem_rmb_amount |临时钱包金额(分) |bigint |0 |不为空 |
|service_amount |服务费金额(分) |bigint |0 |不为空 |
|before_rmb_coins |操作前 组织钱包的剩余费用 |bigint |0 |不为空 |
|before_tem_rmb_coins |操作前 临时钱包的剩余费用 |bigint |0 |不为空 |
...
|wallet_virtual_id |钱包虚拟id |bigint |null |不为空 |
|uuid |所属用户/组织id |bigint |null |不为空 |
|refund_type |类型 tem = 临时,rmb = 钱包, tem_and_rmb=临时钱包与钱包 |varchar |null |不为空 |
|order_id |订单id |varchar |null |是 |
|amount |该订单退款总金额(分) |bigint |0 |不为空 |
|rmb_amount |钱包扣除金额(分) |bigint |0 |不为空 |
|tem_rmb_amount |临时钱包扣除金额(分) |bigint |0 |不为空 |
|refund_status |退款状态  0 - 初始化;1 - 申请退款; 2 - 退款成功 ; 3 - 拒绝退款; |int |0 |不为空 |
|check_type |审核方式 (AUTO-自动  HAND-手动) |varchar |AUTO |是 |
|check_comment |审核评语 |varchar |null |是 |
|check_by |操作者 id |bigint |null |不为空 |
|before_rmb_coins |操作前 组织钱包的剩余费用 |bigint |0 |不为空 |
|before_tem_rmb_coins |操作前 临时钱包的剩余费用 |bigint |0 |不为空 |
