...
```shell
docker run --restart always --name carb-api   -p 18087:8080/tcp -v /home/java/carb/app.jar:/app.jar  -v /home/java/carb/app_logs:/logs/app_logs -v /etc/localtime:/etc/localtime  -it -d adoptopenjdk/openjdk8-openj9:jdk8u292-b10_openj9-0.26.0-centos  java -jar -Dfile.encoding=utf-8 /app.jar  --server.port=8080 -Duser.timezone=GMT+08
```
...
### nginx 配置文件
 ...
 xie2.alicbin.com.conf

```nginx
              
   server {
        listen  80;              #配置监听端口和主机名称
        server_name  xie2.alicbin.com;	
        location /{
                proxy_pass http://127.0.0.1:18087;  #直接
    ...
    server {
	        listen 443 ssl;
	        server_name xie2.alicbin.com;
			ssl_certificate	/home/ssl/xie2.alicbin.com.pem;
	        #ssl_certificate			/cert/rc-pt.cn/server.crt;
	        ssl_certificate_key	/home/ssl/xie2.alicbin.com.key;
