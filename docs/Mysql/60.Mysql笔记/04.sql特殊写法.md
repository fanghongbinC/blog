...
1.需要更改微信支付方式   H5 -> APP 类型   不再需要微信服务号  微信开发平台创建app应用

2.需要更支付宝支付方式   H5 -> APP 类型   支付开发平台 创建 app 类型应用 并签约 APP支付
...
1.  后端调用 苹果 api接口 生成签名字符串 

   注意的重点 ( 好像苹果内购前端支付成功后没有  后台是没有成功异步回调 )

   2.前端使用签名直接调用方法( 要不要插件,我不太清楚)
